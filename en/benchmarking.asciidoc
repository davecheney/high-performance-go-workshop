...
----
NOTE: `go test` will also run all the tests in a package before matching benchmarks, so if you have a lot of tests in a package, or they take a long time to run, you can exclude them by providing `go test`'s `-run` flag with a regex that matches nothing; ie. `go test -run=^$`. 
...
`benchstat` tells us the mean is 44.3 microseconds with a +/- 6% variation across the samples.
This is not unexpected on battery power. 
...
On all versions of the Go compiler, the loop is still generated.
But Intel CPUs are really good at optimising loops, especially empty ones. 

==== Exercise, look at the asmbley
