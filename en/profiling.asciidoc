...
The mutex profile does not show you how long the program ran for, or even what took the most time.
Instead it reports how much time was spent waiting for locks.
...
[source,go,options="nowrap"]
----
include::../examples/mutex/mutex_test.go[tags=mutex]
----
...
====
_Question_: is `b.ResetTimer` also expensive? Does that matter?
====
